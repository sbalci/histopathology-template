...
```{r inspectdf column types 2}
inspectdf::inspect_types(starwars) %>% inspectdf::show_plot()
...
```{r summarytool frequencies html render}
...
```{r explore 3, fig.height=explore::total_fig_height(iris)}
iris %>% 
  explore::explore_all()
```


```{r explore 4, eval=FALSE, fig.height=explore::total_fig_height(iris, target = Species), explore::total_fig_height(iris, target=}
...
```{r SmartEDA variable importance based on Information value}
